...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    const blogCards = await Promise.all(allBlogs.map(async (blog, idx) => {
  ...
  if (searchForm && searchInput) {
    searchForm.addEventListener("submit", function(e) {
      ...
      if (!keyword) {
        // Hiển thị lại tất cả nếu rỗng
        Promise.all(allBlogs.map(async (blog, idx) => {
          const { name, avatar } = await getUserInfo(blog.postedBy);
          return `
          <div class="col-md-6 col-lg-4" data-aos="fade-up" data-aos-delay="${100 + idx*100}">
            <div class="blog-card">
      ...
      if (filtered.length === 0) {
        ...
        Promise.all(filtered.map(async (blog, idx) => {
          const { name, avatar } = await getUserInfo(blog.postedBy);
          return `
          <div class="col-md-6 col-lg-4" data-aos="fade-up" data-aos-delay="${100 + idx*100}">
            <div class="blog-card">
          ...
          row.innerHTML = filteredCards.join("");
          if (window.AOS) AOS.refresh();
        });
      }
    });
    // Nếu xóa hết input thì cũng load lại tất cả bài viết
